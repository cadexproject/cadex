...
void CChainLocksHandler::TrySignChainTip()
{
    ...
    if (IsNewInstaCADEXEnabled() && sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        while (pindexWalk) {
            ...
            for (auto& txid : *txids) {
                ...
                if (txAge < WAIT_FOR_ISLOCK_TIMEOUT && !quorumInstaCADEXManager->IsLocked(txid)) {
