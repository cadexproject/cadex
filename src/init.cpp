...
void PrepareShutdown()
{
    ...
    if (!fLiteMode && !fRPCInWarmup) {
        ...
        if(fEnableInstaCADEX)
        {
            CFlatDB<CInstaCADEX> flatdb5("instantsend.dat", "magicInstaCADEXCache");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-zmqpubhashtxlock=<address>", _("Enable publish hash transaction (locked via InstaCADEX) in <address>"));
    ...
    strUsage += HelpMessageOpt("-zmqpubhashinstantsenddoublespend=<address>", _("Enable publish transaction hashes of attempted InstaCADEX double spend in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawblock=<address>", _("Enable publish raw block in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtx=<address>", _("Enable publish raw transaction in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtxlock=<address>", _("Enable publish raw transaction (locked via InstaCADEX) in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawinstantsenddoublespend=<address>", _("Enable publish raw transactions of attempted InstaCADEX double spend in <address>"));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode", strprintf(_("Disable all Cadex specific functionality (Masternodes, PrivateSend, InstaCADEX, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N CADEX anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
    ...
    strUsage += HelpMessageGroup(_("InstaCADEX options:"));
    strUsage += HelpMessageOpt("-enableinstantsend", strprintf(_("Enable InstaCADEX, show confirmations for locked transactions (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-instantsendnotify=<cmd>", _("Execute command when a wallet InstaCADEX transaction is successfully locked (%s in cmd is replaced by TxID)"));
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://www.cadexcoin.tech>";

    return CopyrightHolders(_("Copyright (C)"), 2014, COPYRIGHT_YEAR) + "\n" +
           "\n" +
           strprintf(_("A special thankyou to Aphunu Edirin Michael, Serik, Unick, Luka, zerocool, gpenco, in_the_know, carbon2000, fozzyblob, Macish, dijida and to all those we forgot.. \n\n"
...
/** Sanity checks
 *  Ensure that Cadex is running in a usable environment with all
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    // ********************************************************* Step 10b: setup InstaCADEX

    fEnableInstaCADEX = GetBoolArg("-enableinstantsend", 1);
    ...
    if (!fIgnoreCacheFiles) {
        ...
        if(fEnableInstaCADEX)
        {
            ...
            uiInterface.InitMessage(_("Loading InstaCADEX data cache..."));
            CFlatDB<CInstaCADEX> flatdb5(strDBName, "magicInstaCADEXCache");
            if(!flatdb5.Load(instantsend)) {
                return InitError(_("Failed to load InstaCADEX data cache from") + "\n" + (pathDB / strDBName).string());
            }
        }
    }
    ...
    if (!fLiteMode) {
        ...
        scheduler.scheduleEvery(boost::bind(&CInstaCADEX::DoMaintenance, boost::ref(instantsend)), 60 * 1000);
