...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, UniValue& entry)
{
    ...
    bool fLocked = instantsend.IsLockedInstaCADEXTransaction(txid);
    bool fLLMQLocked = llmq::quorumInstaCADEXManager->IsLocked(txid);
...
UniValue sendrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "3. instantsend    (boolean, optional, default=false) Use InstaCADEX to send this transaction\n"
    ...
    bool fInstaCADEX = false;
    if (request.params.size() > 2)
        fInstaCADEX = request.params[2].get_bool();
    ...
    if (!fHaveMempool && !fHaveChain) {
        ...
        if (fInstaCADEX && !instantsend.ProcessTxLockRequest(*tx, *g_connman)) {
            throw JSONRPCError(RPC_TRANSACTION_ERROR, "Not a valid InstaCADEX transaction, see debug.log for more info");
        }
