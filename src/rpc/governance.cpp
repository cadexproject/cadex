...
void gobject_prepare_help(CWallet* const pwallet)
{
    throw std::runtime_error(
                ...
                "5. use-IS        (boolean, optional, default=false) InstaCADEX lock the collateral, only requiring one chain confirmation\n"
...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 CADEX irreversibly.
