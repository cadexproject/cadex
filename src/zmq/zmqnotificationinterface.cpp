...
CZMQNotificationInterface* CZMQNotificationInterface::Create()
{
    ...
    factories["pubhashinstantsenddoublespend"] = CZMQAbstractNotifier::Create<CZMQPublishHashInstaCADEXDoubleSpendNotifier>;
    ...
    factories["pubrawinstantsenddoublespend"] = CZMQAbstractNotifier::Create<CZMQPublishRawInstaCADEXDoubleSpendNotifier>;
...
void CZMQNotificationInterface::NotifyInstaCADEXDoubleSpendAttempt(const CTransaction &currentTx, const CTransaction &previousTx)
{
    for (auto it = notifiers.begin(); it != notifiers.end();) {
        CZMQAbstractNotifier *notifier = *it;
        if (notifier->NotifyInstaCADEXDoubleSpendAttempt(currentTx, previousTx)) {
            ++it;
        } else {
            notifier->Shutdown();
