...
void WalletTxToJSON(const CWalletTx& wtx, UniValue& entry)
{
    ...
    bool fLocked = instantsend.IsLockedInstaCADEXTransaction(wtx.GetHash());
    bool fLLMQLocked = llmq::quorumInstaCADEXManager->IsLocked(wtx.GetHash());
...
static void SendMoney(CWallet * const pwallet, const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew, bool fUseInstaCADEX = false, bool fUsePrivateSend = false)
{
    ...
    if (!pwallet->CreateTransaction(vecSend, wtxNew, reservekey, nFeeRequired, nChangePosRet,
                                         strError, NULL, true, fUsePrivateSend ? ONLY_DENOMINATED : ALL_COINS, fUseInstaCADEX)) {
    ...
    if (fUseInstaCADEX && llmq::IsOldInstaCADEXEnabled()) {
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            "6. \"use_is\"             (bool, optional, default=false) Send this transaction as InstaCADEX\n"
    ...
    bool fUseInstaCADEX = false;
    bool fUsePrivateSend = false;
    if (request.params.size() > 5)
        fUseInstaCADEX = request.params[5].get_bool();
    if (request.params.size() > 6)
        fUsePrivateSend = request.params[6].get_bool();

    EnsureWalletIsUnlocked(pwallet);

    SendMoney(pwallet, address.Get(), nAmount, fSubtractFeeFromAmount, wtx, fUseInstaCADEX, fUsePrivateSend);
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "3. addlocked           (bool, optional, default=false) Whether to include transactions locked via InstaCADEX.\n"
            "\nResult:\n"
    ...
    for (const std::pair<uint256, CWalletTx>& pairWtx : pwallet->mapWallet) {
        ...
        BOOST_FOREACH(const CTxOut& txout, wtx.tx->vout)
            if (txout.scriptPubKey == scriptPubKey)
                if ((wtx.GetDepthInMainChain() >= nMinDepth) || (fAddLocked && wtx.IsLockedByInstaCADEX()))
...
UniValue getbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "3. addlocked      (bool, optional, default=false) Whether to include the value of transactions locked via InstaCADEX in the wallet's balance.\n"
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            "5. addlocked         (bool, optional, default=false) Whether to include transactions locked via InstaCADEX.\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "4. addlocked               (bool, optional, default=false) Whether to include transactions locked via InstaCADEX.\n"
            ...
            "      ,...\n"
            "    ]\n"
            "7. \"use_is\"                (bool, optional, default=false) Send this transaction as InstaCADEX\n"
    ...
    bool fUsePrivateSend = false;
    if (request.params.size() > 6)
        fUseInstaCADEX = request.params[6].get_bool();
    if (request.params.size() > 7)
        fUsePrivateSend = request.params[7].get_bool();

    bool fCreated = pwallet->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, nChangePosRet, strFailReason,
                                               NULL, true, fUsePrivateSend ? ONLY_DENOMINATED : ALL_COINS, fUseInstaCADEX);
    if (!fCreated)
...
UniValue ListReceived(CWallet * const pwallet, const UniValue& params, bool fByAccounts)
{
    ...
    for (const std::pair<uint256, CWalletTx>& pairWtx : pwallet->mapWallet) {
        ...
        if ((nDepth < nMinDepth) && !(fAddLocked && wtx.IsLockedByInstaCADEX()))
            continue;
...
UniValue listreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "2. addlocked         (bool, optional, default=false) Whether to include transactions locked via InstaCADEX.\n"
            ...
            "                                                 configured for transactions locked via InstaCADEX\n"
...
void ListTransactions(CWallet * const pwallet, const CWalletTx& wtx, const std::string& strAccount, int nMinDepth, bool fLong, UniValue& ret, const isminefilter& filter)
{
    ...
    // Received
    if (listReceived.size() > 0 && ((wtx.GetDepthInMainChain() >= nMinDepth) || wtx.IsLockedByInstaCADEX()))
    {
        BOOST_FOREACH(const COutputEntry& r, listReceived)
        {
            std::string account;
