...
bool fEnableInstaCADEX = true;

std::atomic<bool> CInstaCADEX::isAutoLockBip9Active{false};
const double CInstaCADEX::AUTO_IX_MEMPOOL_THRESHOLD = 0.1;

CInstaCADEX instantsend;
const std::string CInstaCADEX::SERIALIZATION_VERSION_STRING = "CInstaCADEX-Version-1";

// Transaction Locks
//
// step 1) Some node announces intention to lock transaction inputs via "txlockrequest" message (ix)
// step 2) Top nInstaCADEXSigsTotal masternodes per each spent outpoint push "txlockvote" message (txlvote)
// step 3) Once there are nInstaCADEXSigsRequired valid "txlockvote" messages (txlvote) per each spent outpoint
...
void CInstaCADEX::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    if (fLiteMode) return; // disable all Cadex specific functionality
    if (!llmq::IsOldInstaCADEXEnabled()) return;

    // NOTE: NetMsgType::TXLOCKREQUEST is handled via ProcessMessage() in net_processing.cpp

    if (strCommand == NetMsgType::TXLOCKVOTE) { // InstaCADEX Transaction Lock Consensus Votes
...
bool CInstaCADEX::ProcessTxLockRequest(const CTxLockRequest& txLockRequest, CConnman& connman)
{
    LOCK(cs_main);
#ifdef ENABLE_WALLET
    ...
    for (const auto& txin : txLockRequest.tx->vin) {
        ...
        if (it != mapLockedOutpoints.end() && it->second != txLockRequest.GetHash()) {
            ...
            LogPrintf("CInstaCADEX::ProcessTxLockRequest -- WARNING: Found conflicting completed Transaction Lock, txid=%s, completed lock txid=%s\n",
        ...
        if (it != mapVotedOutpoints.end()) {
            for (const auto& hash : it->second) {
                if (hash != txLockRequest.GetHash()) {
                    LogPrint("instantsend", "CInstaCADEX::ProcessTxLockRequest -- Double spend attempt! %s\n", txin.prevout.ToStringShort());
                    ...
                    if (itPrevious != mapTxLockCandidates.end() && itPrevious->second.txLockRequest) {
                        ...
                        GetMainSignals().NotifyInstaCADEXDoubleSpendAttempt(txCurrent, txPrevious);
                    }
                }
            }
        }
    ...
    if (!CreateTxLockCandidate(txLockRequest)) {
        // smth is not right
        LogPrintf("CInstaCADEX::ProcessTxLockRequest -- CreateTxLockCandidate failed, txid=%s\n", txHash.ToString());
        return false;
    }
    LogPrintf("CInstaCADEX::ProcessTxLockRequest -- accepted, txid=%s\n", txHash.ToString());
...
bool CInstaCADEX::CreateTxLockCandidate(const CTxLockRequest& txLockRequest)
{
    ...
    if (itLockCandidate == mapTxLockCandidates.end()) {
        LogPrintf("CInstaCADEX::CreateTxLockCandidate -- new, txid=%s\n", txHash.ToString());
        ...
        if (itLockCandidate->second.IsTimedOut()) {
            LogPrintf("CInstaCADEX::CreateTxLockCandidate -- timed out, txid=%s\n", txHash.ToString());
            return false;
        }
        LogPrintf("CInstaCADEX::CreateTxLockCandidate -- update empty, txid=%s\n", txHash.ToString());
        ...
        LogPrint("instantsend", "CInstaCADEX::CreateTxLockCandidate -- seen, txid=%s\n", txHash.ToString());
    }

    return true;
}

void CInstaCADEX::CreateEmptyTxLockCandidate(const uint256& txHash)
{
    if (mapTxLockCandidates.find(txHash) != mapTxLockCandidates.end())
        return;
    LogPrintf("CInstaCADEX::CreateEmptyTxLockCandidate -- new, txid=%s\n", txHash.ToString());
...
void CInstaCADEX::Vote(const uint256& txHash, CConnman& connman)
{
...
void CInstaCADEX::Vote(CTxLockCandidate& txLockCandidate, CConnman& connman)
{
    ...
    for (auto& outpointLockPair : txLockCandidate.mapOutPointLocks) {
        ...
        if (nPrevoutHeight == -1) {
            LogPrint("instantsend", "CInstaCADEX::Vote -- Failed to find UTXO %s\n", outpointLockPair.first.ToStringShort());
            return;
        }

        int nLockInputHeight = nPrevoutHeight + Params().GetConsensus().nInstaCADEXConfirmationsRequired - 2;
        ...
        if (!CMasternodeUtils::GetMasternodeRank(activeMasternodeInfo.outpoint, nRank, quorumModifierHash, nLockInputHeight)) {
            LogPrint("instantsend", "CInstaCADEX::Vote -- Can't calculate rank for masternode %s\n", activeMasternodeInfo.outpoint.ToStringShort());
            continue;
        }

        int nSignaturesTotal = Params().GetConsensus().nInstaCADEXSigsTotal;
        if (nRank > nSignaturesTotal) {
            LogPrint("instantsend", "CInstaCADEX::Vote -- Masternode not in the top %d (%d)\n", nSignaturesTotal, nRank);
            continue;
        }

        LogPrint("instantsend", "CInstaCADEX::Vote -- In the top %d (%d)\n", nSignaturesTotal, nRank);
        ...
        if (itVoted != mapVotedOutpoints.end()) {
            for (const auto& hash : itVoted->second) {
                ...
                if (it2->second.HasMasternodeVoted(outpointLockPair.first, activeMasternodeInfo.outpoint)) {
                    ...
                    // skip it anyway
                    fAlreadyVoted = true;
                    LogPrintf("CInstaCADEX::Vote -- WARNING: We already voted for this outpoint, skipping: txHash=%s, outpoint=%s\n",
        ...
        if (!vote.Sign()) {
            LogPrintf("CInstaCADEX::Vote -- Failed to sign consensus vote\n");
            return;
        }
        if (!vote.CheckSignature()) {
            LogPrintf("CInstaCADEX::Vote -- Signature invalid\n");
            return;
        }
        ...
        if (outpointLockPair.second.AddVote(vote)) {
            LogPrintf("CInstaCADEX::Vote -- Vote created successfully, relaying: txHash=%s, outpoint=%s, vote=%s\n",
            ...
            if (itVoted == mapVotedOutpoints.end()) {
                ...
                if (mapVotedOutpoints[outpointLockPair.first].size() > 1) {
                    ...
                    LogPrintf("CInstaCADEX::Vote -- WARNING: Vote conflicts with some existing votes: txHash=%s, outpoint=%s, vote=%s\n",
                            txHash.ToString(), outpointLockPair.first.ToStringShort(), nVoteHash.ToString());
                }
            }
...
bool CInstaCADEX::ProcessNewTxLockVote(CNode* pfrom, const CTxLockVote& vote, CConnman& connman)
{
    ...
    if (!vote.IsValid(pfrom, connman)) {
        ...
        LogPrint("instantsend", "CInstaCADEX::%s -- Vote is invalid, txid=%s\n", __func__, txHash.ToString());
        return false;
    }
    ...
    if (it == mapTxLockCandidates.end() || !it->second.txLockRequest) {
        ...
        LogPrint("instantsend", "CInstaCADEX::%s -- Orphan vote: txid=%s  masternode=%s %s\n",
        ...
        if (itMnOV == mapMasternodeOrphanVotes.end()) {
            ...
            if (itMnOV->second > GetTime() && itMnOV->second > GetAverageMasternodeOrphanVoteTime()) {
                LogPrint("instantsend", "CInstaCADEX::%s -- masternode is spamming orphan Transaction Lock Votes: txid=%s  masternode=%s\n",
    ...
    if (txLockCandidate.IsTimedOut()) {
        LogPrint("instantsend", "CInstaCADEX::%s -- too late, Transaction Lock timed out, txid=%s\n", __func__, txHash.ToString());
        return false;
    }

    LogPrint("instantsend", "CInstaCADEX::%s -- Transaction Lock Vote, txid=%s\n", __func__, txHash.ToString());
    ...
    LogPrint("instantsend", "CInstaCADEX::%s -- Transaction Lock signatures count: %d/%d, vote hash=%s\n", __func__,
...
bool CInstaCADEX::ProcessOrphanTxLockVote(const CTxLockVote& vote)
{
...
void CInstaCADEX::UpdateVotedOutpoints(const CTxLockVote& vote, CTxLockCandidate& txLockCandidate)
{
    AssertLockHeld(cs_instantsend);
        ...
        for (const auto& hash : it1->second) {
            if (hash != txHash) {
                ...
                if (it2 !=mapTxLockCandidates.end() && it2->second.HasMasternodeVoted(vote.GetOutpoint(), vote.GetMasternodeOutpoint())) {
                    ...
                    LogPrintf("CInstaCADEX::%s -- masternode sent conflicting votes! %s\n", __func__, vote.GetMasternodeOutpoint().ToStringShort());
...
void CInstaCADEX::ProcessOrphanTxLockVotes()
{
    AssertLockHeld(cs_main);
    AssertLockHeld(cs_instantsend);
...
void CInstaCADEX::TryToFinalizeLockCandidate(const CTxLockCandidate& txLockCandidate)
{
    if (!llmq::IsOldInstaCADEXEnabled()) return;
    ...
    if (txLockCandidate.IsAllOutPointsReady() && !IsLockedInstaCADEXTransaction(txHash)) {
        // we have enough votes now
        LogPrint("instantsend", "CInstaCADEX::TryToFinalizeLockCandidate -- Transaction Lock is ready to complete, txid=%s\n", txHash.ToString());
...
void CInstaCADEX::UpdateLockedTransaction(const CTxLockCandidate& txLockCandidate)
{
    // cs_main, cs_wallet and cs_instantsend should be already locked
    AssertLockHeld(cs_main);
#ifdef ENABLE_WALLET
    ...
    if (!IsLockedInstaCADEXTransaction(txHash)) return; // not a locked tx, do not update/notify
    ...
    LogPrint("instantsend", "CInstaCADEX::UpdateLockedTransaction -- done, txid=%s\n", txHash.ToString());
}

void CInstaCADEX::LockTransactionInputs(const CTxLockCandidate& txLockCandidate)
{
    if (!llmq::IsOldInstaCADEXEnabled()) return;
    ...
    LogPrint("instantsend", "CInstaCADEX::LockTransactionInputs -- done, txid=%s\n", txHash.ToString());
}

bool CInstaCADEX::GetLockedOutPointTxHash(const COutPoint& outpoint, uint256& hashRet)
{
    LOCK(cs_instantsend);
...
bool CInstaCADEX::ResolveConflicts(const CTxLockCandidate& txLockCandidate)
{
    AssertLockHeld(cs_main);
    AssertLockHeld(cs_instantsend);
    ...
    for (const auto& txin : txLockCandidate.txLockRequest.tx->vin) {
        ...
        if (GetLockedOutPointTxHash(txin.prevout, hashConflicting) && txHash != hashConflicting) {
            ...
            if (itLockCandidate == mapTxLockCandidates.end() || itLockCandidateConflicting == mapTxLockCandidates.end()) {
                ...
                LogPrintf("CInstaCADEX::ResolveConflicts -- ERROR: Found conflicting completed Transaction Lock, but one of txLockCandidate-s is missing, txid=%s, conflicting txid=%s\n",
                        txHash.ToString(), hashConflicting.ToString());
                return false;
            }
            LogPrintf("CInstaCADEX::ResolveConflicts -- WARNING: Found conflicting completed Transaction Lock, dropping both, txid=%s, conflicting txid=%s\n",
                    txHash.ToString(), hashConflicting.ToString());
            ...
            LogPrintf("CInstaCADEX::ResolveConflicts -- ERROR: Failed to complete Transaction Lock, conflicts with mempool, txid=%s\n", txHash.ToString());
            return false;
        }
    } // FOREACH
    ...
    if (GetTransaction(txHash, txTmp, Params().GetConsensus(), hashBlock, true) && hashBlock != uint256()) {
        LogPrint("instantsend", "CInstaCADEX::ResolveConflicts -- Done, %s is included in block %s\n", txHash.ToString(), hashBlock.ToString());
        return true;
    }
    ...
    for (const auto& txin : txLockCandidate.txLockRequest.tx->vin) {
        ...
        if (!GetUTXOCoin(txin.prevout, coin)) {
            ...
            LogPrintf("CInstaCADEX::ResolveConflicts -- ERROR: Failed to find UTXO %s, can't complete Transaction Lock\n", txin.prevout.ToStringShort());
            return false;
        }
    }
    LogPrint("instantsend", "CInstaCADEX::ResolveConflicts -- Done, txid=%s\n", txHash.ToString());

    return true;
}

int64_t CInstaCADEX::GetAverageMasternodeOrphanVoteTime()
{
    LOCK(cs_instantsend);
...
void CInstaCADEX::CheckAndRemove()
{
    if (!masternodeSync.IsBlockchainSynced()) return;
    ...
    while (itLockCandidate != mapTxLockCandidates.end()) {
        ...
        if (txLockCandidate.IsExpired(nCachedBlockHeight)) {
            LogPrintf("CInstaCADEX::CheckAndRemove -- Removing expired Transaction Lock Candidate: txid=%s\n", txHash.ToString());
    ...
    while (itVote != mapTxLockVotes.end()) {
        if (itVote->second.IsExpired(nCachedBlockHeight)) {
            LogPrint("instantsend", "CInstaCADEX::CheckAndRemove -- Removing expired vote: txid=%s  masternode=%s\n",
    ...
    while (itOrphanVote != mapTxLockVotesOrphan.end()) {
        if (itOrphanVote->second.IsTimedOut()) {
            LogPrint("instantsend", "CInstaCADEX::CheckAndRemove -- Removing timed out orphan vote: txid=%s  masternode=%s\n",
    ...
    while (itVote != mapTxLockVotes.end()) {
        if (itVote->second.IsFailed()) {
            LogPrint("instantsend", "CInstaCADEX::CheckAndRemove -- Removing vote for failed lock attempt: txid=%s  masternode=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetMasternodeOutpoint().ToStringShort());
            mapTxLockVotes.erase(itVote++);
        } else {
            ++itVote;
    ...
    while (itMasternodeOrphan != mapMasternodeOrphanVotes.end()) {
        if (itMasternodeOrphan->second < GetTime()) {
            LogPrint("instantsend", "CInstaCADEX::CheckAndRemove -- Removing timed out orphan masternode vote: masternode=%s\n",
    ...
    LogPrintf("CInstaCADEX::CheckAndRemove -- %s\n", ToString());
}

bool CInstaCADEX::AlreadyHave(const uint256& hash)
{
    if (!llmq::IsOldInstaCADEXEnabled()) {
        return true;
    }
...
void CInstaCADEX::AcceptLockRequest(const CTxLockRequest& txLockRequest)
{
    LOCK(cs_instantsend);
    mapLockRequestAccepted.insert(std::make_pair(txLockRequest.GetHash(), txLockRequest));
}

void CInstaCADEX::RejectLockRequest(const CTxLockRequest& txLockRequest)
{
    LOCK(cs_instantsend);
    mapLockRequestRejected.insert(std::make_pair(txLockRequest.GetHash(), txLockRequest));
}

bool CInstaCADEX::HasTxLockRequest(const uint256& txHash)
{
    CTxLockRequest txLockRequestTmp;
    return GetTxLockRequest(txHash, txLockRequestTmp);
}

bool CInstaCADEX::GetTxLockRequest(const uint256& txHash, CTxLockRequest& txLockRequestRet)
{
    if (!llmq::IsOldInstaCADEXEnabled()) {
        return false;
    }
...
bool CInstaCADEX::GetTxLockVote(const uint256& hash, CTxLockVote& txLockVoteRet)
{
    if (!llmq::IsOldInstaCADEXEnabled()) {
        return false;
    }
...
bool CInstaCADEX::IsLockedInstaCADEXTransaction(const uint256& txHash)
{
    if (!fEnableInstaCADEX || GetfLargeWorkForkFound() || GetfLargeWorkInvalidChainFound() ||
...
int CInstaCADEX::GetTransactionLockSignatures(const uint256& txHash)
{
    if (!fEnableInstaCADEX) return -1;
    if (GetfLargeWorkForkFound() || GetfLargeWorkInvalidChainFound()) return -2;
    if (!llmq::IsOldInstaCADEXEnabled()) return -3;
...
bool CInstaCADEX::IsTxLockCandidateTimedOut(const uint256& txHash)
{
    if (!fEnableInstaCADEX) return false;
...
void CInstaCADEX::Relay(const uint256& txHash, CConnman& connman)
{
    LOCK(cs_instantsend);
...
void CInstaCADEX::UpdatedBlockTip(const CBlockIndex *pindex)
{
    nCachedBlockHeight = pindex->nHeight;
}

void CInstaCADEX::SyncTransaction(const CTransaction& tx, const CBlockIndex *pindex, int posInBlock)
{
    ...
    LogPrint("instantsend", "CInstaCADEX::SyncTransaction -- txid=%s nHeightNew=%d\n", txHash.ToString(), nHeightNew);

    // Check lock candidates
    std::map<uint256, CTxLockCandidate>::iterator itLockCandidate = mapTxLockCandidates.find(txHash);
    if (itLockCandidate != mapTxLockCandidates.end()) {
        LogPrint("instantsend", "CInstaCADEX::SyncTransaction -- txid=%s nHeightNew=%d lock candidate updated\n",
                txHash.ToString(), nHeightNew);
        ...
        for (const auto& pair : itLockCandidate->second.mapOutPointLocks) {
            ...
            for (const auto& vote : pair.second.GetVotes()) {
                uint256 nVoteHash = vote.GetHash();
                LogPrint("instantsend", "CInstaCADEX::SyncTransaction -- txid=%s nHeightNew=%d vote %s updated\n",
...
bool CInstaCADEX::CanAutoLock()
{
    if (!isAutoLockBip9Active || !llmq::IsOldInstaCADEXEnabled()) {
        return false;
    }
...
bool CTxLockRequest::IsValid() const
{
    ...
    int nInstaCADEXConfirmationsRequired = Params().GetConsensus().nInstaCADEXConfirmationsRequired;
    ...
    for (const auto& txin : tx->vin) {
        ...
        int nConfirmationsRequired = nInstaCADEXConfirmationsRequired - 1;
...
{
    if (!fForceMinFee && CInstaCADEX::CanAutoLock() && IsSimple()) {
        return CAmount();
    }
...
{
    return tx->vin.size() * Params().GetConsensus().nInstaCADEXSigsTotal;
}
...
bool CTxLockVote::IsValid(CNode* pnode, CConnman& connman) const
{
    ...
    int nLockInputHeight = coin.nHeight + Params().GetConsensus().nInstaCADEXConfirmationsRequired - 2;
...
{
    // Locks and votes expire nInstaCADEXKeepLock blocks after the block corresponding tx was included into.
    return (nConfirmedHeight != -1) && (nHeight - nConfirmedHeight > Params().GetConsensus().nInstaCADEXKeepLock);
}
...
{
    return (GetTime() - nTimeCreated > INSTANTSEND_FAILED_TIMEOUT_SECONDS) && !instantsend.IsLockedInstaCADEXTransaction(GetTxHash());
}
...
{
    return !fAttacked && CountVotes() >= Params().GetConsensus().nInstaCADEXSigsRequired;
}
