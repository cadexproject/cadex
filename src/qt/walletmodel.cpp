...
bool WalletModel::IsOldInstaCADEXEnabled() const
{
    return llmq::IsOldInstaCADEXEnabled();
}
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    if(recipients[0].fUseInstaCADEX && IsOldInstaCADEXEnabled() && total > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
        Q_EMIT message(tr("Send Coins"), tr("InstaCADEX doesn't support sending values that high yet. Transactions are currently limited to %1 CADEX.").arg(sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)),
    ...
    std::string strFailReason;
    {
        ...
        fCreated = wallet->CreateTransaction(vecSend, *newTx, *keyChange, nFeeRequired, nChangePosRet, strFailReason, coinControl, true, recipients[0].inputType, recipients[0].fUseInstaCADEX);
    ...
    if(recipients[0].fUseInstaCADEX && IsOldInstaCADEXEnabled()) {
        ...
        if(nVinSize > CTxLockRequest::WARN_MANY_INPUTS) {
            Q_EMIT message(tr("Send Coins"), tr("Used way too many inputs (>%1) for this InstaCADEX transaction, fees could be huge.").arg(CTxLockRequest::WARN_MANY_INPUTS),
...
bool WalletModel::transactionCanBeAbandoned(uint256 hash) const
{
    ...
    if (!wtx || wtx->isAbandoned() || wtx->GetDepthInMainChain() > 0 || wtx->IsLockedByInstaCADEX() || wtx->InMempool())
        return false;
    return true;
}
